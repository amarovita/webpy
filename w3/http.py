...
from . import net, utils, webapi as w3
...
def expires(delta):
    ...
    w3.header('Expires', net.httpdate(date_obj))

def lastmodified(date_obj):
    """Outputs a `Last-Modified` header for `datetime`."""
    w3.header('Last-Modified', net.httpdate(date_obj))
...
def modified(date=None, etag=None):
    ...
    n = set([x.strip('" ') for x in w3.ctx.env.get('HTTP_IF_NONE_MATCH', '').split(',')])
    m = net.parsehttpdate(w3.ctx.env.get('HTTP_IF_MODIFIED_SINCE', '').split(';')[0])
    validate = False
    if etag:
    ...
    if etag: w3.header('ETag', '"' + etag + '"')
    if validate:
        raise w3.notmodified()
    else:
        return True
